...
public class Coll11 : MonoBehaviour {

	private List<string> ColliderBlock11 = new List<string> {"One", "LetA", "Square", "Checker","Two", "LetB", "Circle", "Stripe","Three", "LetC", "Triangle", "Polkadot","Four", "LetD", "Star", "Diagonal","Five", "LetE", "Plus", "Target"};
	...
	void Update () {
		if (Physics2D.OverlapCircle(this.transform.position,0.7f) == true) {
			...
			if (peg.name.Contains(ColliderBlock11[PlayerPrefs.GetInt("indexkey")]) || peg.name.Contains(ColliderBlock11[(PlayerPrefs.GetInt("Answers10")-1)*4+PlayerPrefs.GetInt("indexkey1")])) {
