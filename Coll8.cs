...
public class Coll8 : MonoBehaviour {

	private List<string> ColliderBlock8 = new List<string> {"Three", "LetC", "Green", "Triangle", "Polkadot"};
	...
	void Update () {
		if (Physics2D.OverlapCircle(this.transform.position,0.7f) == true) {
			...
			if (peg.name.Contains (ColliderBlock8[PlayerPrefs.GetInt("indexkey1")])){
