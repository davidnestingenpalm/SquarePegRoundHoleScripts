...
public class ResetConfirmScript : MonoBehaviour {
    ...
    public GameObject Title, Subtitle, Back, Reset;
	...
	void Start () {
		...
		style1.fontSize = (int) Mathf.Floor(Screen.height*0.08f);
    ...
    void SceneSizer() {
        ...
        Reset.GetComponent<RectTransform>().sizeDelta = new Vector2(safeUIWidth*widthAdj,safeUIHeight*heightAdj*(ratio-ratioAdj));
    ...
    void SceneLayout() {
		Resetpos.x = safeMidX;
		Resetpos.y = safeMinY + safeHeight*0.4f;
	...
	void OnGUI () {
		...
		if (Reseting.name.Contains ("easy")) {
			GUI.Label (new Rect (safeMinX,(pixelsy - safeMaxY) + safeHeight*0.33f,safeWidth,safeHeight*0.1f),"Reset all Easy Level scores?",style1);
		}
		if (Reseting.name.Contains ("medium")) {
			GUI.Label (new Rect (safeMinX,(pixelsy - safeMaxY) + safeHeight*0.33f,safeWidth,safeHeight*0.1f),"Reset all Medium Level scores?",style1);
		}
		if (Reseting.name.Contains ("hard")) {
			GUI.Label (new Rect (safeMinX,(pixelsy - safeMaxY) + safeHeight*0.33f,safeWidth,safeHeight*0.1f),"Reset all Hard Level scores?",style1);
		}
		if (Reseting.name.Contains ("expert")) {
			GUI.Label (new Rect (safeMinX,(pixelsy - safeMaxY) + safeHeight*0.33f,safeWidth,safeHeight*0.1f),"Reset all Expert Level scores?",style1);
		}
		if (Reseting.name.Contains ("insane")) {
			GUI.Label (new Rect (safeMinX,(pixelsy - safeMaxY) + safeHeight*0.33f,safeWidth,safeHeight*0.1f),"Reset all Insane Level scores?",style1);
		}
		if (Reseting.name.Contains ("game")) {
			GUI.Label (new Rect (safeMinX,(pixelsy - safeMaxY) + safeHeight*0.33f,safeWidth,safeHeight*0.1f),"Reset the entire game?",style1);
		}
	}
